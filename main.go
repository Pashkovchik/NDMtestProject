...
func main() {
	...
	// Канал ожидания для закрытия соединений
	idleConnectionsClosed := make(chan struct{})

	// Ожидание сигнала SIGINT для graceful shutdown
	go func() {
		sigint := make(chan os.Signal, 1)
		signal.Notify(sigint, os.Interrupt)
		<-sigint
		// Ожидание, когда все запросы завершатся
...
// queueHandlerConfig - конфигурация для обработчиков запросов
type queueHandlerConfig struct {
	store *store
	pool  *channelPool
}

// функция queueHandler обеспечивает разделение обработки по методам запроса
...
// putQueueHandler обработчик метода PUT, который кладёт данные в очередь
func (c *queueHandlerConfig) putQueueHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Попытка отправить сообщение обработчикам. Если нет обработчиков ожидающих сообщения, то кладём в хранилище
...
// getQueueHandler обработчик метода GET на получение данных из очереди.
func (c *queueHandlerConfig) getQueueHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Попытка получить сообщение из хранилища
	msg, ok := c.store.pop(name)
	// Если нет сообщений и не нужно ждать сообщения, то выводим ошибку 404.
	if !ok && !hasTimeout {
	...
	if hasTimeout {
		...
		// Инициализируем канал, по которому получаем сообщение
		...
		// Обработка в зависимости от того, что придёт раньше (таймаут или сообщение)
		select {
		case <-timeoutTimer.C:
		...
		case msg := <-ch:
			// Удаляем из очереди, так как сообщение получено
			c.pool.removeFromQueue(name, ch)
...
// push кладет ключ и сообщение в хранилище. Если сообщение уже есть, то новое сообщение добавится в конец
...
// pop получает первый элемент ключа с удалением его из хранилища
...
// channelPool хранилище каналов, которые принимают сообщения для их вывода
...
// sendMessage отправляет сообщения первому каналу из очереди
...
func getQueueName(url *url.URL) (string, bool) {
	// Разбираем путь на строки, чтобы получить имя очереди
	// Если слайс не равен 2, то имя очереди указано не верно или его нет
